...
// Iterative 3-simplex builder class.
class Simplex {
    Array<Vec3f, 4> m_vertices;

public:
    explicit Simplex(const Vec3f &vertex) : m_vertices{vertex} {}

    const Vec3f &operator[](uint32_t index) const { return m_vertices[index]; }
};
...
Optional<Contact> Context::run() const {
    ...
    // b) Create a simplex with the first vertex being the support from this initial direction.
    auto direction = vull::normalise(md_center);
    Simplex simplex(support_point(direction));

    // Next direction is towards the origin.
    direction = vull::normalise(-simplex[0]);
    direction = -md_center;
    ...
    if (vull::dot(md_center, direction) > 0.0f) {
        // If origin is on the - side of the plane, reverse the direction.
        vull::swap(v1, v2);
        direction = -direction;
    }
