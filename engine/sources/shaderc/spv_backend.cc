...
AccessChain Backend::lower_call_expr(const hir::CallExpr &call_expr) {
    ...
    if (call_expr.callee().kind() == hir::NodeKind::ExtInst) {
        const auto &ext_inst = static_cast<const hir::ExtInst &>(call_expr.callee());
    ...
    VULL_ASSERT(call_expr.callee().kind() == hir::NodeKind::FunctionDecl);
    auto *callee = *m_function_map.get(static_cast<const hir::FunctionDecl *>(&call_expr.callee()));
