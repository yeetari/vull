...
LegaliseResult<hir::FunctionDecl> Legaliser::lower_function_decl(const ast::FunctionDecl &ast_decl) {
    ...
    for (uint32_t i = 0; i < m_pipeline_decls.size(); i++) {
        ...
        VULL_TRY(create_pipeline_variable(symbol.name(), symbol.type(), i));
    }
...
Result<void, LegaliseError> Legaliser::lower_io_decl(const ast::IoDecl &ast_decl) {
...
/*Result<void, LegaliseError> Legaliser::lower_uniform_decl(const ast::Aggregate &ast_decl) {
    VULL_ASSERT(ast_decl.kind() == ast::AggregateKind::UniformBlock);
    for (const auto &field : ast_decl.nodes()) {
        auto variable = m_root.allocate<hir::BindingVariable>();
        variable->set_type(field->type());

        const auto &symbol = static_cast<const ast::Symbol &>(*field);
        VULL_TRY(m_scope->put_symbol(symbol.name(), vull::move(variable)));
    }
    return {};
}*/
...
Result<void, LegaliseError> Legaliser::lower_top_level(const ast::Node &ast_node) {
    switch (ast_node.kind()) {
        ...
        /*case Aggregate:
        VULL_TRY(lower_uniform_decl(static_cast<const ast::Aggregate &>(ast_node)));
        return {};*/
    case FunctionDecl: {
