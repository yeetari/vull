...
// The legaliser turns the AST into a high-level IR (HIR) which has a similar structure to the AST but with the
// following changes:
// 1) Symbols and lexical scoping in general is removed
// 2) All expressions become typed
// 3) Some higher level binary operations, e.g. multiply turn into more specific operations, e.g. MatrixTimesVector
