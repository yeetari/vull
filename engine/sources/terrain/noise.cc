...
const Array<uint8_t, 256> s_perm{
...
uint32_t noise(uint32_t x, uint32_t z, uint32_t seed) {
    uint32_t tmp = s_perm[(x + seed) % 256];
    return s_perm[(tmp + z) % 256];
}

float smooth_lerp(float a, float b, float x) {
    return vull::lerp(b, a, x * x * (3.0f - 2.0f * x));
}

} // namespace

float perlin_noise(float x, float z, uint32_t seed) {
    auto x_int = static_cast<uint32_t>(x);
    auto z_int = static_cast<uint32_t>(z);
    auto s = static_cast<float>(noise(x_int, z_int, seed));
    auto t = static_cast<float>(noise(x_int + 1, z_int, seed));
    auto u = static_cast<float>(noise(x_int, z_int + 1, seed));
    auto v = static_cast<float>(noise(x_int + 1, z_int + 1, seed));
    float lo = smooth_lerp(s, t, x - static_cast<float>(x_int));
    float hi = smooth_lerp(u, v, x - static_cast<float>(x_int));
    return smooth_lerp(lo, hi, z - static_cast<float>(z_int)) / 256.0f;
}
