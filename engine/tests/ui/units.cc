...
TEST_CASE(UiUnits, LayoutUnitFloat) {
    EXPECT_THAT(LayoutUnit::from_float_pixels(0.0f).to_float(), is(close_to(0.0f, LayoutUnit::epsilon())));
    EXPECT_THAT(LayoutUnit::from_float_pixels(100000.0f).to_float(), is(close_to(100000.0f, LayoutUnit::epsilon())));
    EXPECT_THAT(LayoutUnit::from_float_pixels(-100000.0f).to_float(), is(close_to(-100000.0f, LayoutUnit::epsilon())));

    EXPECT_THAT(LayoutUnit::from_float_pixels(1.0f / 3.0f).to_float(),
                is(close_to(1.0f / 3.0f, LayoutUnit::epsilon())));
    EXPECT_THAT(LayoutUnit::from_float_pixels(-1.0f / 3.0f).to_float(),
                is(close_to(-1.0f / 3.0f, LayoutUnit::epsilon())));
    EXPECT_THAT(-LayoutUnit::from_float_pixels(1.0f / 3.0f).to_float(),
                is(close_to(-1.0f / 3.0f, LayoutUnit::epsilon())));
    EXPECT_THAT(-LayoutUnit::from_float_pixels(-1.0f / 3.0f).to_float(),
                is(close_to(1.0f / 3.0f, LayoutUnit::epsilon())));
    EXPECT_THAT((LayoutUnit::from_float_pixels(1.0f / 3.0f) * 3).to_float(), is(close_to(1.0f, LayoutUnit::epsilon())));
    EXPECT_THAT((LayoutUnit::from_float_pixels(-1.0f / 3.0f) * 3).to_float(),
                is(close_to(-1.0f, LayoutUnit::epsilon())));
    EXPECT_THAT((LayoutUnit::from_float_pixels(1.0f / 3.0f) * -3).to_float(),
                is(close_to(-1.0f, LayoutUnit::epsilon())));
}
...
TEST_CASE(UiUnits, LayoutUnitScaleBy) {
    EXPECT_THAT(LayoutUnit::from_int_pixels(10).scale_by(1.0f / 3.0f).to_float(),
                is(close_to(10.0f / 3.0f, LayoutUnit::epsilon())));
    EXPECT_THAT(LayoutUnit::from_int_pixels(10).scale_by(-1.0f / 3.0f).to_float(),
                is(close_to(-10.0f / 3.0f, LayoutUnit::epsilon())));
}
