...
struct bc7enc_compress_block_params {
    ...
    // m_max_partitions may range from 0 (disables mode 1) to BC7ENC_MAX_PARTITIONS. The higher this value, the slower
    // the compressor, but the higher the quality.
    uint32_t m_max_partitions;

    // Relative RGBA or YCbCrA weights.
    uint32_t m_weights[4];

    // m_uber_level may range from 0 to BC7ENC_MAX_UBER_LEVEL. The higher this value, the slower the compressor, but the
    // higher the quality.
    ...
    // When m_mode17_partition_estimation_filterbank, the mode1 partition estimator skips lesser used partition patterns
...
// Packs a single block of 16x16 RGBA pixels (R first in memory) to 128-bit BC7 block pBlock, using either mode 1
// and/or 6. Alpha blocks will always use mode 6, and by default opaque blocks will use either modes 1 or 6. Returns
// true if the block had any pixels with alpha < 255, otherwise it return false. (This is not an error code - a block is
// always encoded.)
